...
void TextChannelListener::handleMessages()
{
    ...
    qDebug() << __PRETTY_FUNCTION__ << "Number of messages in local message queue: " << m_messageQueue.size();
...
CommHistory::Group TextChannelListener::getGroupById(int groupId) const
{
    ...
    if (m_Group.isValid() && m_Group.id() == groupId) {
        group = m_Group;
    } else if (!m_GroupModel || !m_GroupModel->isReady()) {
        qWarning() << Q_FUNC_INFO << "Can't read group model";
    } else {
        for (int row = 0; row < m_GroupModel->rowCount(); row++) {
            QModelIndex index = m_GroupModel->index(row, 0);
            CommHistory::Group g = m_GroupModel->group(index);
            if (g.isValid() && g.id() == groupId) {
                group = g;
                break;
            }
        }
    ...
    if (group.isValid()) {
        CommHistory::Group::PropertySet props = group.validProperties();
        // m_GroupModel doesn't track contact changes; don't send invalid data.
        props -= CommHistory::Group::Contacts;
        group.setValidProperties(props);
    } else {
