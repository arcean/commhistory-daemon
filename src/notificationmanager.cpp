...
void NotificationManager::removeMissedCallNotifications(const QString &localId,
                                                        const QString &remoteId)
{
    qDebug() << Q_FUNC_INFO << localId << remoteId;
    ...
    while (i.hasNext()) {
        ...
        if ((eventType == CommHistory::Event::CallEvent)
                ...
                && CommHistory::remoteAddressMatch(i.value().remoteUid(), remoteId)) {
            m_unresolvedEvents.removeAll(i.value());
            i.remove();
            updated = true;
        }
    ...
    if (updated) {
        qDebug() << Q_FUNC_INFO << "updating group";
        clearContactsCache();
        updateNotificationGroup(notificationGroup(CommHistory::Event::CallEvent));
        saveState();
    }
}
...
void NotificationManager::fireNotifications()
{
    ...
    QList<NotificationGroup> keys = m_Notifications.uniqueKeys();
    foreach(NotificationGroup ng ,keys) {
        // if the eventtype is unknown, we omit it
        if (ng.type() == CommHistory::Event::UnknownType) {
            clearPendingEvents(ng);
        } else {
            if (ng.isValid()) {
